...
If you have some datasets already installed somewhere else, you can create symbolic links in `$DATA/dataset_name` that point to the original data to avoid duplicate download. The instruction: ln -s <target> <link_name>
...
|   |–– val/
```symbolic links
- If you had downloaded the ImageNet dataset before, you can create  to map the training and validation sets to `$DATA/imagenet/images`. 
